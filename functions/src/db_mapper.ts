    ...
    for (const idx in rs){
        ...
        for (const doc of docs){
            try {
                ...
                ps2.push(processCollections(qantyDb, subCollections, result, ignoredPaths, allowedToReadAllDocs))
            } catch (error) {
                console.log("Aquí muere esta flor")
            }
        }
    }
