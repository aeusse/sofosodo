...
function drawItem(idx, item){
    indexHtml += `<br><div class="index_item"><a href="#item_${idx}"><b>${idx+1} &nbsp;&nbsp; ${item.title}</b></a></div>`;
    fullHtml += `<div id="item_${idx}"><h2><center>${item.title}</center></h2></div>`;
...
function drawSubItem(idx, currentPath, subItem){
    ...
    indexHtml += `<div class="index_item"><a href="#sub_item_${currentPathText}${idx}">${indent}${currentPathText}${(idx+1)} &nbsp;&nbsp; ${subItem.title}</a></div>`;
    fullHtml += `<div id="sub_item_${currentPathText}${idx}"><h4><center>${subItem.title}</center></h4></div>`;
